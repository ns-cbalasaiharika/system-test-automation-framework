...
const config = loadConfig('bl10-kafka-throughput');
...
export function setup(): SetupData {
  ...
  console.log(`[${config.scenario.name}] Setup: Seeding test data for Kafka throughput test...`);
  const seededIds = seedConfigs(config.baseUrl, config.headers, 5);
